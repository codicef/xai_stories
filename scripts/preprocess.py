...
def main():
    ...
    # ── METABRIC-only assignment (Francesco, Ziyun, André) ───────
    # 3 users × 25 cases = 75 assignments over 100 METABRIC cases.
    # Interleaved stride-3 indices so all three users sample the
    # same spread of the distribution.
    METABRIC_EVALUATORS = [
        'Francesco Codicè',
        'Ziyun Pan',
        'André Mas',
    ...
    metabric_assignment = {
        user: [c['id'] for c in metabric_cases[i::3][:25]]
        for i, user in enumerate(METABRIC_EVALUATORS)
    }

    cov_m = Counter(cid for ids in metabric_assignment.values() for cid in ids)
    print(f'\nMETABRIC-only assignment ({len(METABRIC_EVALUATORS)} evaluators, 25 cases each):')
    ...
    # 4 users × 25 cases = 100 assignments over 100 MM cases:
    #   → every MM case seen by exactly 1 evaluator (perfect coverage).
    MM_EVALUATORS = [
        'Caroline Bret',
    ...
    n_eval = len(MM_EVALUATORS)   # 4
    block = n_mm // n_eval        # 25 cases per user (exact)

    mm_assignment = {
        user: [c['id'] for c in mm_cases[i * block : (i + 1) * block]]
    ...
    print(f'\nMM-only assignment ({n_eval} evaluators, {block} cases each):')
